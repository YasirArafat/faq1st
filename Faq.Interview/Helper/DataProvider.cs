    ...
    public class DataProvider
    {
        ...
        public List<Employee> Report(int? month)
        {
            ...
            foreach (var employee in employees)
            {
                ...
                employee.WeeklySalary = GetWeeklySalary(employee.Salaries);
            }

            return employees;
        }

        private WeeklySalary GetWeeklySalary(List<Salary> salaries)
        {
            WeeklySalary weeklySalary = new WeeklySalary();
            foreach (Salary salary in salaries)
            {
                List<int> bounds = new List<int>() { 1, 8, 9, 17, 31, int.MaxValue };
                switch ((salary.Date.Day >= 1 && salary.Date.Day <= 7) ? "Week1" :
                        (salary.Date.Day >= 8 && salary.Date.Day <= 14) ? "Week2" :
                        (salary.Date.Day >= 15 && salary.Date.Day <= 21) ? "Week3" :
                        (salary.Date.Day >= 22 && salary.Date.Day <= 28) ? "Week4" : "Week5")
                {
                    case "Week1":
                        weeklySalary.Week1Salary += salary.Amount;
                        break;
                    case "Week2":
                        weeklySalary.Week2Salary += salary.Amount;
                        break;
                    case "Week3":
                        weeklySalary.Week3Salary += salary.Amount;
                        break;
                    case "Week4":
                        weeklySalary.Week4Salary += salary.Amount;
                        break;
                    case "Week5":
                        weeklySalary.Week5Salary += salary.Amount;
                        break;
                }
            }
            return weeklySalary;
